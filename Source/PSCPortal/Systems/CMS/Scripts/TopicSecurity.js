...
function LoadDataCallback(results, context, methodName) {
    ...
    DrawTitle(authenList.TopicPermissionList);    
    DrawData(authenList.TopicPermissionList, authenList.RoleList, authenList.AuthenticationList);
}

function pageLoad(){    
    LoadData();
}
function DrawTitle(topicPermissionList)
{
    ...
    for( var index in topicPermissionList)
    {
        var $item=$("<div></div>");
        $item.text(topicPermissionList[index].Name);
...
function DrawData(topicPermissionList, roleList, authenticationList)
{
    for ( var i = 0 ; i < roleList.length ; i++)
    {
        AddRowForRole(roleList[i], topicPermissionList, authenticationList[i]);
    }
}
function AddRowForRole(role, topicPermissionList, arrAuthenticationList){
    var flag=true;
    ...
    for( var i = 0 ; i < topicPermissionList.length ; i++)
    {
        var $item=$("<div></div>");
